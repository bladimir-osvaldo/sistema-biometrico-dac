...
// Dirección del servidor. IMPORTANTE:
//  - NO uses "localhost" (eso apunta al propio ESP32).
//  - Usa la IP de tu PC en la red local, por ejemplo "192.168.1.50".
//    Para saber tu IP: Windows → abre "cmd" → escribe:  ipconfig
//    Busca "Dirección IPv4".
const String API_BASE = "http://192.168.1.50/PROYECTO-DAC/sistema-biometrico-dac/public/api/v1";

// Token del lector. Se obtiene así:
//   Inicia sesión como Admin → "Lectores ESP32" → "Nuevo Lector" →
//   al guardar se genera un token (cópialo aquí, sin espacios).
const String API_TOKEN   = "PEGA_AQUI_EL_TOKEN_DEL_LECTOR";

// Estos datos los completas al crear el lector en el panel web:
const String DEVICE_MAC      = "24:6F:28:AA:BB:CC";
...
/* ════════════════════════════════════════════════════════════════════
   ENROLAMIENTO DE UNA HUELLA (proceso en 3 pasadas de captura)
   ════════════════════════════════════════════════════════════════════ */
...
bool esperarDedo(unsigned long tiempoMs) {
  ...
  while (millis() - inicio < tiempoMs) {
    char k = keypad.getKey();
    if (k == '*') return false;                    // cancelar con *
...
uint8_t enrolar(int id) {
  ...
  // Pasada 1
  lcdPrint("Dedo 1 de 3", "Coloque su dedo");
  if (!esperarDedo(30000)) return FINGERPRINT_PACKETRECIEVEERR;
  p = finger.image2Tz(1);
  if (p != FINGERPRINT_OK) return p;
  lcdPrint("Dedo 1 de 3", "Retire el dedo");
  esperarRetiro();
  delay(300);

  // Pasada 2
  lcdPrint("Dedo 2 de 3", "Coloque su dedo");
  if (!esperarDedo(30000)) return FINGERPRINT_PACKETRECIEVEERR;
  p = finger.image2Tz(2);
  if (p != FINGERPRINT_OK) return p;
  lcdPrint("Dedo 2 de 3", "Retire el dedo");
  esperarRetiro();
  delay(300);

  // Pasada 3
  lcdPrint("Dedo 3 de 3", "Coloque su dedo");
  if (!esperarDedo(30000)) return FINGERPRINT_PACKETRECIEVEERR;
  p = finger.image2Tz(3);
  if (p != FINGERPRINT_OK) return p;
  lcdPrint("Dedo 3 de 3", "Retire el dedo");
  esperarRetiro();
  delay(300);
...
void modoRegistrar() {
  ...
  // 3) Enrolar la huella en el sensor (3 pasadas)
  ...
  if (resp.length() > 3 && resp.indexOf("\"success\":true") >= 0) {
    ...
    if (fpCount < MAX_FP) {
      fpIds[fpCount] = id;
      strncpy(fpNames[fpCount], "Docente " + String(codigo), 39);
      fpNames[fpCount][39] = '\0';
      fpCount++;
    }
...
void escanearYMarcar() {
  ...
  lcdPrint("Ponga su dedo", "Tecla * cancela");
  digitalWrite(LED_AMARILLO, HIGH);

  // Esperar el dedo hasta 20 segundos
  bool dedo = false;
  while (millis() - inicio < 20000) {
    char k = keypad.getKey();
    if (k == '*') { digitalWrite(LED_AMARILLO, LOW); return; }
    p = finger.getImage();
    if (p == FINGERPRINT_OK) { dedo = true; break; }
    delay(50);
  }
  ...
  if (p == FINGERPRINT_NOFINGER) {
    digitalWrite(LED_AMARILLO, LOW);
    lcdPrint("No se detecto", "dedo");
    delay(1200);
    return;
  }

  p = finger.image2Tz();
  if (p != FINGERPRINT_OK) { digitalWrite(LED_AMARILLO, LOW); return; }
  ...
  if (p == FINGERPRINT_OK) {
    modoMarcar(finger.fingerID);     // ¡Coincidencia! → marcar
  } else {
    digitalWrite(LED_ROJO, HIGH);
    beep(3, 200);
    lcdPrint("NO REGISTRADA", "Huella no valida");
    delay(2500);
    digitalWrite(LED_ROJO, LOW);
    lcdPrint("Listo para marcar", "Coloque su dedo");
  }
